...
EXPLAINATION
      ...
      variant - standard, full, cloud, variant-gnome,
                variant-kde, variant-lxde, variant-xfce
...
EXAMPLE
 	 ./build.sh build standard amd64 2.1
"
}
...
function build() {
	...
	lb config
	lb build
	mv live-image-*.hybrid.iso `date +Parrot-$variant-3.0.%y%m%d_$arch.iso`
}
