...
function lts_amd64 {
	...
	EDITION=lts
	echo "Building $EDITION-$ARCH"
	sudo debootstrap --arch=$ARCH --components=main,contrib,non-free --include=gnupg2,nano --exclude=tasksel,parrot-core $EDITION $EDITION-$ARCH https://deb.parrot.sh/mirrors/parrot/ > $EDITION-$ARCH.log
	echo "Customizing $EDITION-$ARCH"
	sudo rm -rf $EDITION-$ARCH/var/cache/apt/*
	echo "Importing $EDITION-$ARCH in docker"
	sudo tar -C $EDITION-$ARCH -c . | docker import - parrotsec/core:base-$EDITION-$ARCH
